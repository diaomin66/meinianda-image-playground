...
import { buildCanvasAgentTextConfig, buildCanvasGalleryImageConfig } from "@canvas/lib/gallery-image-bridge";
...
export function buildGenerationConfig(config: AiConfig, node: CanvasNodeData | undefined, mode: CanvasNodeGenerationMode): AiConfig {
    if (mode === "image") return buildCanvasGalleryImageConfig(config, node?.metadata);
    if (mode === "text") return buildCanvasAgentTextConfig(config);
    return {
        ...config,
