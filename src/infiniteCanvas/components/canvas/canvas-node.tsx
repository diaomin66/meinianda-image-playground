...
import { CanvasNodeType, type CanvasNodeData, type Position } from "@canvas/types/canvas";
...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    const selectionBlue = theme.canvas.selectionStroke;
    ...
    return (
        ...
        >
            ...
            <div
                className={`relative h-full w-full overflow-visible rounded-3xl ${hasImageContent ? "border-0" : "border-2"}`}
                style={{
                    ...
                    boxShadow: isGroupDropTarget ? `0 0 0 2px ${selectionBlue}66, inset 0 0 0 999px ${selectionBlue}10` : isActive ? `0 0 0 2px ${selectionBlue}` : isRelated && !isBatchChild ? `0 0 0 1px ${theme.node.muted}55, 0 18px 48px rgba(0,0,0,.14)` : undefined,
                }}
            ...
            >
                <div
                    className={`relative flex h-full w-full items-center justify-center rounded-[inherit] [backface-visibility:hidden] [transform:translateZ(0)] ${isBatchRoot ? "overflow-visible" : "overflow-hidden"}`}
                    style={
                        {
        ...
        <BatchFrame batchCount={isBatchRoot ? batchCount : 0} batchExpanded={batchExpanded} batchOpening={batchOpening} batchRecovering={batchRecovering} onToggleBatch={onToggleBatch}>
            <div className="h-full w-full">
                <img
                    ...
                    className={`pointer-events-none block h-full w-full select-none [backface-visibility:hidden] ${node.metadata?.freeResize ? "object-fill" : "object-contain"}`}
                />
            </div>
            {isBatchRoot ? (
                <button
                    type="button"
                className="absolute right-2.5 top-2.5 z-30 flex h-8 items-center justify-center gap-1 rounded-full border px-2.5 text-xs font-semibold shadow-[0_6px_18px_rgba(24,24,27,.10)] backdrop-blur-md transition hover:scale-[1.02]"
            ...
            {isBatchChild ? (
                <button
                    type="button"
                    className="absolute right-3 top-3 z-30 flex h-9 items-center gap-1.5 rounded-xl border px-2.5 text-xs font-medium opacity-0 shadow-[0_8px_20px_rgba(63,63,70,.13)] backdrop-blur-md transition group-hover/batch:opacity-100 hover:scale-[1.02]"
                    style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text }}
                    onClick={(event) => {
                        event.stopPropagation();
                        onSetBatchPrimary?.();
            ...
            {isBatchRoot ? (
                <div className="pointer-events-none absolute inset-0 overflow-visible">
                    {Array.from({ length: Math.min(batchCount - 1, 5) }).map((_, index) => (
                        <div
                            key={index}
                        className="absolute rounded-[inherit] border shadow-[0_14px_34px_rgba(63,63,70,.16)] transition-all duration-300 group-hover/batch:translate-x-2"
                            style={{
                                inset: 0,
