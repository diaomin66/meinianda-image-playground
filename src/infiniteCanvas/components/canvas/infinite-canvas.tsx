...
export function InfiniteCanvas({ containerRef, viewport, backgroundMode = "lines", onViewportChange, onCanvasMouseDown, onCanvasDeselect, onCanvasDoubleClick, onContextMenu, onDrop, children }: InfiniteCanvasProps) {
    ...
    const viewportX = Math.round(viewport.x * pixelRatio) / pixelRatio;
    const viewportY = Math.round(viewport.y * pixelRatio) / pixelRatio;
    ...
    return (
        <div
            ...
            onDrop={onDrop}
        >
            <CanvasGrid viewport={{ ...viewport, x: viewportX, y: viewportY }} mode={backgroundMode} />
            <div
            ...
            >
                <div className="relative origin-top-left" style={{ zoom: viewport.k }}>
                    {children}
                </div>
            </div>
        </div>
