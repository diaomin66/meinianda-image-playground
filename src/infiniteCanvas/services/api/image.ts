...
import { requestCanvasGalleryImages } from "@canvas/lib/gallery-image-bridge";
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    const galleryImages = await requestCanvasGalleryImages(config, prompt, [], undefined, options?.signal);
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    const galleryImages = await requestCanvasGalleryImages(config, prompt, references, mask, options?.signal);
    if (galleryImages) return galleryImages;
    const requestConfig = resolveModelRequestConfig(config, config.model || config.imageModel);
    const n = Math.max(1, Math.min(15, Math.floor(Math.abs(Number(config.count)) || 1)));
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    const requestConfig = config.galleryTextProfileId ? config : resolveModelRequestConfig(config, config.model || config.textModel);
    const script = config.galleryTextProfileId ? "" : resolveModelScript(config, config.model || config.textModel);
    if (script) {
        try {
