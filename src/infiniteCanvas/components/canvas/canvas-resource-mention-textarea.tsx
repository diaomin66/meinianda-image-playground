        ...
        <div className={`relative h-full w-full ${containerClassName || ""}`}>
            {showOverlay ? (
                <div ref={overlayRef} className={`${className || ""} pointer-events-none absolute inset-0 overflow-hidden whitespace-pre-wrap break-words`} style={{ ...style, color: theme.node.text }}>
                    <MentionHighlightText value={value || props.placeholder?.toString() || ""} labels={activeLabels} placeholder={!value} accent={theme.node.activeStroke} />
                </div>
            ) : null}
            <textarea
                {...props}
...
function MentionHighlightText({ value, labels, placeholder, accent }: { value: string; labels: string[]; placeholder: boolean; accent: string }) {
        ...
        <>
            {value.split(pattern).map((part, index) =>
                labels.includes(part) ? (
                    <span key={`${part}-${index}`} className="rounded-md px-1 py-0.5 font-medium ring-1" style={{ color: accent, background: `${accent}18`, boxShadow: `inset 0 0 0 1px ${accent}30` }}>
                        {part}
                    </span>
                ) : (
