...
import { CircleDot, Eraser, Grid2x2, Group, Hand, Image as ImageIcon, Info, Palette, Puzzle, Redo2, Settings2, Square, Trash2, Type, Undo2, Upload } from "lucide-react";

import { canvasThemes, type CanvasBackgroundMode, type CanvasTheme } from "@canvas/lib/canvas-theme";
...
export function CanvasToolbar({
    ...
    const dockStyle = { background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.item, boxShadow: colorTheme === "dark" ? "0 18px 45px rgba(0,0,0,.32)" : "0 16px 40px rgba(24,24,27,.12)" };
    ...
    return (
        <div ref={rootRef} className="canvas-workspace-toolbar pointer-events-none absolute bottom-5 z-50 flex justify-center" style={{ left: 300, right: 16 }}>
            {tip ? <DockTip label={tip} x={tipX} theme={theme} /> : null}
            <div ref={wrapRef} className="canvas-workspace-toolbar-scroll thin-scrollbar pointer-events-auto flex h-14 max-w-full items-center gap-1 overflow-x-auto rounded-xl border px-2 shadow-lg backdrop-blur [&>*]:shrink-0" style={dockStyle}>
