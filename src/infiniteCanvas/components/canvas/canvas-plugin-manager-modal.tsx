...
export function CanvasPluginManagerModal({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    // 图标外挂一个主色点(右上角),用于「有可升级版本」的提示。
    // boxShadow 画一圈与卡片同色的描边环,让提示点从图标上「浮起」。
    const withUpgradeDot = (icon: ReactNode) => (
        <span className="relative inline-flex">
            {icon}
            <span className="absolute -right-1 -top-1 size-2 rounded-full" style={{ background: theme.node.activeStroke, boxShadow: `0 0 0 2px ${theme.node.fill}` }} title="有新版本可升级" />
        </span>
    );
    ...
    return (
        <Modal getContainer={getCanvasOverlayHost} title="节点插件" open={open} onCancel={onClose} footer={null} centered width={640}>
            <div className="space-y-3">
                <div className="flex items-start gap-2 rounded-lg border border-yellow-500/30 bg-yellow-500/10 px-3 py-2 text-xs leading-5" style={{ color: theme.node.text }}>
