...
function CanvasNodesTab({ nodes, selectedNodeIds, onFocusNode, onPreviewNode, theme }: { nodes: CanvasNodeData[]; selectedNodeIds: Set<string>; onFocusNode: (nodeId: string) => void; onPreviewNode: (nodeId: string) => void; theme: CanvasTheme }) {
    ...
    return (
        <div className="flex h-full flex-col">
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 px-3 pb-3 pt-2">
                <div className="flex min-w-0 items-center gap-1.5">
                    <span className="whitespace-nowrap text-xs font-medium opacity-60">画布元素</span>
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                    <button
                        ...
                        className="flex min-h-9 items-center gap-1 rounded-lg px-2.5 text-xs font-medium opacity-70 transition hover:bg-black/5 hover:opacity-100 dark:hover:bg-white/10"
                        style={selectMode ? { color: theme.toolbar.activeText, opacity: 1 } : undefined}
                    >
                        <ListChecks className="size-3.5" />
                        <span className="whitespace-nowrap">{selectMode ? "取消" : "选择"}</span>
                    </button>
                    {selectMode ? null : <Select size="middle" variant="borderless" className="h-9 w-20 [&_.ant-select-selector]:!h-9 [&_.ant-select-selection-item]:!leading-9" value={typeFilter} onChange={setTypeFilter} options={NODE_FILTER_OPTIONS} />}
                </div>
            </div>
            <div className="px-3 pb-2.5">
                <Input size="middle" className="!h-9" allowClear prefix={<Search className="size-3.5 text-stone-400" />} placeholder="搜索节点" value={keyword} onChange={(e) => setKeyword(e.target.value)} />
            </div>
    ...
    return (
        <div className="flex h-full flex-col">
            <div className="flex items-center gap-2 px-3 pb-2 pt-1">
                <Input size="middle" className="!h-9" allowClear prefix={<Search className="size-3.5 text-stone-400" />} placeholder="搜索资产" value={keyword} onChange={(e) => setKeyword(e.target.value)} />
                <button
                    type="button"
                    disabled={uploading}
                    onClick={() => fileInputRef.current?.click()}
                    className="flex min-h-9 shrink-0 items-center gap-1 rounded-lg px-2.5 text-xs font-semibold transition hover:bg-black/5 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-white/10"
                    style={{ color: theme.node.text }}
                >
    ...
    return (
        <div className="flex h-full flex-col">
            <div className="px-3 pb-2.5 pt-1">
                <Input size="middle" className="!h-9" allowClear prefix={<Search className="size-3.5 text-stone-400" />} placeholder="搜索提示词" value={keyword} onChange={(e) => setKeyword(e.target.value)} />
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
                <div className="space-y-1">
